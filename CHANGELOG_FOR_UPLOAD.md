...
- The new `CHANGELOG_FOR_UPLOAD.md` (this file) should be included in the repo root for the PR; it is intentionally concise. If you'd like more detail (per-file diffs), I can generate a more verbose changelog.

Ready-for-PR checklist (suggested)
- [ ] Run interactive app and exercise Modeling/Exploratory flows end-to-end with representative datasets.
- [ ] Run unit tests (if any) and address any failing tests.
- [ ] Optionally add a nicer loading spinner in `ui/analytics_tab._show_loading` (UX enhancement).

If you want, I can also create a small Git commit message template and prepare the repo for a PR (add a branch, commit the changes, and generate the PR text).